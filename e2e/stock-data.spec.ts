...
test.describe('法人持股、預算查詢、查詢紀錄', () => {
  ...
  test('預算查詢：結果的價格都不超過上限', async ({ page }) => {
    ...
    await page.locator('.stock-app input.budget-input, .stock-app input[type=number]').first().fill('200');
    await page.getByRole('button', { name: /查詢|搜尋/ }).click();
  ...
  test('查過的東西出現在查詢紀錄，清除後歸零', async ({ page }) => {
    ...
    await expect(page.getByText(/共 0 筆|沒有紀錄/)).toBeVisible();
  });
});
